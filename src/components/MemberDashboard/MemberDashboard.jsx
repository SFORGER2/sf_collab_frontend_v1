...
export default function MemberDashboard() {
  ...
  const myTasks = [
    { id: 1, title: "Design API architecture", status: "In Progress", color: "text-cyan-400" },
    { id: 2, title: "Fix auth token bug", status: "High Priority", color: "text-orange-400" },
    { id: 3, title: "Write Q3 release notes", status: "Pending review", color: "text-purple-400" },
  ];

  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-[#0a0b10] to-[#11131a] text-slate-300">
      <NavBar />
      <MobileNavBar />
      ...
      <div className="w-full max-w-6xl mx-auto p-6 md:p-8">
        ...
        <div className="flex items-center gap-3 mb-8 border-b border-slate-800 pb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-100">
              <ShinyText text={`Welcome, ${user?.firstName || "Member"} 👋`} speed={3} />
            </h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
          <StatCard title="Execution Score" value="94%" icon={<Target className="text-cyan-400" />} valueColor="text-cyan-400" />
          <StatCard title="Approved Points" value="1,250" icon={<CheckCircle2 className="text-purple-400" />} valueColor="text-purple-400" />
          <StatCard title="Payout Estimate" value="$3,450" icon={<DollarSign className="text-emerald-400" />} valueColor="text-emerald-400" />
          <StatCard title="Attendance" value="98%" icon={<CalendarCheck className="text-blue-400" />} />
          <StatCard title="Active Warnings" value="0" icon={<AlertTriangle className="text-emerald-400" />} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-[#0d0f17] border border-slate-800 rounded-2xl p-6 shadow-sm">
            <h3 className="text-sm font-bold text-slate-200 mb-6 flex items-center gap-2">
              <Activity className="w-4 h-4 text-orange-400" /> Contribution Trend (Last 7 Days)
            </h3>
            <div className="h-48 flex items-end justify-between gap-2 pt-4">
              {weeklyTrend.map((value, index) => (
                <div key={index} className="w-full flex flex-col items-center gap-2 group">
                  <div className="w-full bg-orange-500/20 hover:bg-orange-500/40 border border-orange-500/30 rounded-t-sm transition-all relative" style={{ height: `${value}%` }}>
                  ...
                  <span className="text-[10px] text-slate-500">Day {index + 1}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-1 bg-[#0d0f17] border border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col">
            ...
            <div className="space-y-3 flex-1">
              {myTasks.map((task) => (
                <div key={task.id} className="p-3 bg-[#11131a] border border-slate-800/80 rounded-xl hover:border-slate-700 transition-colors">
                  <p className="text-sm font-medium text-slate-200 mb-1">{task.title}</p>
                  <p className={`text-xs font-semibold ${task.color}`}>{task.status}</p>
                </div>
              ))}
            </div>
