...
import { 
  FileText, Download, Share2, Edit3, Trash2, Clock, Users, 
  Link as LinkIcon, Tag, Eye, ArrowLeft, MoreHorizontal 
} from 'lucide-react';
...
const FileDetailPage = () => {
  const file = {
    ...
    previewUrl: "/api/placeholder/800/520", // Replace with real preview (image/PDF/embed)
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white font-mono overflow-hidden">
      {/* Top Bar - Minimal, matching your SFCollab header style */}
      ...
      <div className="max-w-7xl mx-auto px-8 py-8 grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-8">
          {/* Preview Area - Large glassmorphic card with glow */}
          <div className="relative bg-[#12121a] border border-white/10 rounded-3xl overflow-hidden shadow-2xl shadow-purple-950/50">
            ...
            <div className="p-6 border-t border-white/10 flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-4 text-white/60">
                <div>Current Operator: COMMANDER_ALPHA</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#12121a] border border-white/10 rounded-3xl p-8">
            ...
            <div className="flex flex-wrap gap-2">
              {file.tags.map((tag, i) => (
                <div 
                  key={i}
                  className="px-4 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl text-sm transition-colors cursor-pointer"
                >
                  #{tag}
                </div>
              ))}
        ...
        {/* Right Sidebar - Linked Items + Version History */}
        <div className="col-span-12 lg:col-span-4 space-y-8">
          ...
          <div className="bg-[#12121a] border border-white/10 rounded-3xl p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-xs text-white/40">3 connected</span>
            </div>
            ...
            <div className="space-y-4">
              {file.linkedMilestones.map((milestone) => (
                <div 
                  key={milestone.id}
                  className="group p-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl transition-all flex gap-4"
                >
                  <div className={`w-2 h-2 mt-2 rounded-full flex-shrink-0 ${milestone.status === 'completed' ? 'bg-emerald-400' : milestone.status === 'in-progress' ? 'bg-amber-400' : 'bg-white/30'}`} />
