...
const GroupsChat = () => {
      ...
      try {
        ...
        const convos = Array.isArray(data?.conversations)
          ...
          : Array.isArray(data?.data?.conversations)
            ...
            : [];
        const activeGroups = convos.filter((c) => !c?.is_archived && c.conversation_type === 'group');
        
        if (active) {
          setConversations(activeGroups);
        }
      } catch (err) {
        console.error("GroupsChat failed to fetch:", err);
        if (active) {
          setConversations([]);
        }
      } finally {
  ...
  const handleBroadcast = async (content) => {
    // Send message to all 2 groups simultaneously
    await Promise.all(
